...
from rmac.RoiPooling import RoiPooling
...
def rmac(input_shape, num_rois, model_in):
    ...
    # ROI pooling
    x = RoiPooling([1], num_rois)([model_in.layers[-5].output, in_roi])
...
if __name__ == "__main__":
    ...
    rmacr = rmac((x.shape[1], x.shape[2], x.shape[3]), len(regions),vgg16_model)

    # Compute RMAC vector
    print('Extracting RMAC from image...')
    RMAC = rmacr.predict([x, np.expand_dims(regions, axis=0)])
